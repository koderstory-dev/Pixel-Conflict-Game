    ...
    public class DisplayObjectContainer extends DisplayObject
    {
            ...
            if (index >= 0 && index <= numChildren)
            {
                ...
                else
                {
                    ...
                    if (stage)
                    {
                        ...
                        if (container) container.broadcastEventWith(SEvent.ADDED_TO_STAGE);
                        else           child.dispatchEventWith(SEvent.ADDED_TO_STAGE);
                    }
                }
        ...
        public function removeChildAt(index:int, dispose:Boolean=false):DisplayObject
        {
            if (index >= 0 && index < numChildren)
            {
                ...
                {
                    var container:DisplayObjectContainer = child as DisplayObjectContainer;
                    if (container) container.broadcastEventWith(SEvent.REMOVED_FROM_STAGE);
                    else           child.dispatchEventWith(SEvent.REMOVED_FROM_STAGE);
                }
        ...
        public function broadcastEvent(event:SEvent):void
        {
            if (event.bubbles)
        ...
        {
            var event:SEvent = SEvent.fromPool(type, false, data);
