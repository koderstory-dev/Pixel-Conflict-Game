package starling.display.graphics
{
	...
	public class Graphic extends DisplayObject
	{
		...
		public function Graphic()
		{
			...
			if (Starling.current)
			{
				Starling.current.addEventListener(SEvent.CONTEXT3D_CREATE, onContextCreated);
			}
		}
		
		private function onContextCreated( event:SEvent ):void
		{
		...
		override public function dispose():void
		{
			if (Starling.current)
			{
				Starling.current.removeEventListener(SEvent.CONTEXT3D_CREATE, onContextCreated);
				super.dispose();
			}
